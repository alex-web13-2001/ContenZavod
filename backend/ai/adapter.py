...
FORMAT_INSTRUCTIONS = {
    "flash": (
        "Формат: Молния/Flash. ЖЁСТКИЙ лимит: body должен быть НЕ БОЛЕЕ 200 символов (целевой диапазон 80-180). "
        "Перед отправкой ПОСЧИТАЙ символы. Если получилось >200 — перепиши короче. "
        "Одно-два предложения с ключевым фактом. Начинай тело с эмодзи ⚡. "
        "Весь пост — это ТОЛЬКО голый факт. НЕ добавляй комментарии, выводы, вопросы, call-to-action, жирный текст."
    ),
    "short_post": (
        "Формат: Стандартный пост для Telegram. ЖЁСТКИЙ лимит: body НЕ БОЛЕЕ 450 символов (целевой диапазон 250-420). "
        "Перед отправкой ПОСЧИТАЙ символы. Если получилось >450 — перепиши короче, выкинув второстепенные детали. "
        "Один ключевой факт с кратким контекстом. 2-3 абзаца по 1-2 предложения. "
        "Между абзацами ОБЯЗАТЕЛЬНО пустая строка (\\n\\n). "
    ...
    "longread": (
        "Формат: Аналитика. Целевой диапазон 1000-2200 символов, ЖЁСТКИЙ потолок 2500. "
        "Перед отправкой ПОСЧИТАЙ символы. Если получилось >2500 — сократи. "
        "Развёрнутая статья НА ФАКТАХ: вводка с главным фактом, 2-3 аргумента с конкретными данными/цифрами, вывод. "
...
10. CHARACTER LIMITS ARE A HARD CONSTRAINT, not a guideline. The format instructions specify a strict upper
    bound for the body. Count characters before responding. If you exceed the limit by even 10%, you have
    failed the task — rewrite the body shorter. Prefer cutting secondary details, redundant adjectives,
    and "for context" sentences over breaking the limit.
