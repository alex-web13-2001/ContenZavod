...
### Added
- **LLM-судья как финальный слой дедупа (сравнение смысла, а не слов).** `event_key` оказался ненадёжен: три источника одной истории под разными ракурсами дают три разных ключа с недостаточным пересечением. Добавлен `ai/semantic_dedup.find_duplicate`: перед постановкой кандидата в очередь автопилот спрашивает LLM «это дубль какой-то из опубликованных за 48ч?», передавая заголовок+саммари кандидата и до 12 недавних. Ловит одно событие из разных изданий/языков/ракурсов (на чём валились все эвристики). Вызывается только для финалистов (после всех дешёвых фильтров) — максимум 1 вызов на кандидата; при сбое API «не дубль» (никогда не блокирует публикацию). Проверено на живом API: 3 ракурса DISY/Фидиас → дубль, пожар из другого источника → дубль, несвязная новость → пропуск (3/3).
- **Семантический кросс-языковой дедуп через `event_key`.** Эвристики (общие сущности, слова заголовка, хеш картинки) принципиально не ловили одну историю из разных изданий на разных языках — у греческой и английской статьи про одну кампанию ноль общих слов. Классификатор теперь выдаёт канонический язык-независимый `event_key` (slug реального события): одна история на EN/GR/RU → совпадающий ключ. Дедуп (`compute_uniqueness_score`) сравнивает ключи по пересечению значимых токенов (≥3 общих и ≥60%, со стоп-листом года/гео-шума) — это устойчиво к недетерминизму LLM в хвосте слага (`…-launch` vs `…-2026`). Проверено на живом API: EN+GR про экономию воды → общий ключ (дубль ловится), несвязная новость → 0 пересечения.
...
### Fixed
- **Circuit breaker на генерацию обложек (защита от выжигания кредитов).** При отказе KIE (кончились кредиты / сетевой сбой) две ретрай-петли — `max_retries=5` в задаче + периодический `autopilot_retry_covers` каждые 10 мин — накладывались и долбили API: 14 июня 21 реальная обложка превратилась в ~2500 попыток и сожгла все кредиты за ночь. Добавлен `ai/provider_health` (Redis circuit breaker, кулдаун 30 мин): hard-ошибка (`Credits insufficient`/DNS/conn) трипает breaker и больше НЕ ретраится (сразу `permanently_failed`); `create_image_task` при открытом breaker фейлится мгновенно без вызова API; `autopilot_retry_covers` пропускает весь цикл в кулдаун. Временные ошибки (timeout, content-policy) ретраятся как прежде. Само-восстановление через 30 мин.
