...
### 🤖 Быстрая команда для AI: «обнови доку»

Когда пользователь пишет **«обнови доку»** или **«sync docs»** — AI-ассистент **обязан** выполнить полный цикл:

1. **Аудит:** Прочитать все файлы из таблицы обязательных документов выше
2. **Сверка с кодом:** Сравнить документацию с реальным состоянием:
   - `app/models/` → `docs/DATABASE.md`
   - `app/api/v1/` → `docs/API.md`  
   - `app/services/` + `workers/` → `docs/ARCHITECTURE.md`
   - `ai/` → `docs/AI_PROVIDERS.md`
   - `docker-compose.yml` + `.env.example` → `docs/DEPLOYMENT.md`
3. **Исправить** все расхождения (устаревшие таблицы, отсутствующие эндпоинты, неверные потоки данных)
4. **CHANGELOG.md** — добавить записи о всех незафиксированных изменениях
5. **ADR** — если были значимые архитектурные решения без ADR → создать
6. **README.md** — обновить стек и возможности если изменились
7. **Коммит:** `docs: sync documentation with codebase`
8. **Отчёт:** Показать что обновлено в формате таблицы
