...
| Формат | Описание | Длина | Обложка | Превью ссылок |
|--------|----------|-------|---------|---------------|
| `flash` ⚡ | Один факт/цена/назначение, без заголовка | 80–200 символов | ❌ нет | ❌ отключено |
| `short_post` 📝 | Стандартный TG-пост с кратким контекстом | 250–500 символов | ✅ опц. | ✅ |
| `longread` 📊 | Аналитика с цифрами и аргументами | 1000–2500 символов | ✅ | ✅ |
| `video_script` | Сценарий для короткого видео | 200–400 слов | — | — |
| `digest` | Дайджест-формат (краткие факты) | 100–250 символов | — | — |

- **AI-рекомендация формата:** Классификатор возвращает поле `suggested_format` (`flash`/`short_post`/`longread`). При адаптации этот формат используется, если он есть в `channel.content_formats`; иначе — платформенный дефолт.
